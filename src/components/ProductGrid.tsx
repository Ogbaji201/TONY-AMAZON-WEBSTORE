...
//                 <button
//                   onClick={() => addToCart({ id, name, price, image: img ?? undefined })}
...
function pick<T>(v: T | undefined | null, f: T): T {
  return v ?? f;
}
...
function getImageUrl(p: RawProduct): string | undefined {
  const a = getAttr(p);
  const img = a.Image_Upload;
  let images: any[] = [];
          ...
          >
            ...
            <div className="p-6">
              ...
              <p className="text-gray-600 text-sm mb-4 line-clamp-2">
                {a?.Product_description ?? "No description available."}
              </p>
              ...
              <div className="space-y-3">
                ...
                {/* Add to cart – uses CartContext */}
                <button
                  onClick={() => addToCart({ id, name, price, quantity: 1, image: img })}
                  className={cartBtnClass}
                >
                  <ShoppingCart className="w-4 h-4 mr-2 transition-transform group-hover:-translate-y-0.5" />
                  Add to Cart
