...
//   }, [isMounted, slides.length]); // (ok)

//   if (!slides || slides.length === 0) {
//     return (
//       <div
//         className={`relative w-full ${heightClass} bg-gradient-to-br from-blue-700 to-indigo-700 flex items-center justify-center`}
//       >
...
//           {/* ✅ Link styled as a button (NO nested <button>) */}
//           <Link
//             href="/products"
//             className="inline-flex items-center justify-center bg-white text-blue-600 px-8 py-3 rounded-full font-semibold hover:bg-gray-100 transition"
//           >
...
//                   <Link
//                     href="/products"
//                     className="inline-flex items-center justify-center bg-white text-blue-600 px-6 md:px-8 py-2 md:py-3 rounded-full font-semibold hover:bg-gray-100 transition-all duration-200 transform hover:scale-105 shadow-lg"
//                   >
...
//                 index === current ? 'bg-white shadow-lg scale-125' : 'bg-white/50 hover:bg-white/70'
//               }`}
...
export default function SimpleCarousel({
  ...
  // FIX: wrap next/prev in useCallback so they are stable references
  ...
  // FIX: depend on `next` (stable via useCallback) — no more stale closure
  useEffect(() => {
      ...
      {slides.map((slide, index) => (
        ...
        >
          <div className="relative w-full h-full">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-6 md:p-8 mx-4 max-w-2xl border border-white/10 shadow-xl">
                <div className="text-center text-white px-4">
                  <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold mb-3 drop-shadow-lg leading-tight">
                    {slide.title}
                  </h2>
                  <p className="text-sm md:text-lg lg:text-xl mb-4 drop-shadow-lg leading-relaxed opacity-90">
                    {slide.description}
                  </p>
                  <p className="text-2xl md:text-3xl font-bold text-yellow-300 mb-4 drop-shadow-lg">
                    ₦{Number(slide.price).toLocaleString()}
                  </p>
                  <Link
                    href="/products"
